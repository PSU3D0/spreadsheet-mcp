...
| `verify` | _(none today)_ | CLI_ONLY | `adapter-cli.verify_paths` | n/a | Baseline/current proof surface for target deltas, error provenance, and optional named-range deltas | `crates/spreadsheet-kit/src/cli/commands/verify.rs::verify` | `crates/spreadsheet-kit/tests/cli_integration.rs` |
