...
| `verify_workbook` | `verify` | SHARED_PARTIAL | `core.verify.compare_workbooks` | later | Shared proof contract; MCP compares workbook/fork ids while CLI compares file paths; SDK exposes MCP helpers, WASM parity is later | `crates/spreadsheet-kit/src/tools/mod.rs::verify_workbook` | `crates/spreadsheet-kit/tests/cli_integration.rs` |
