...
| `append-region` | _(none today)_ | CLI_ONLY | `adapter-cli.append_region` | n/a | Region-based append helper that resolves a detected region, accepts JSON rows or CSV rows, inserts before footer-like totals when found, then compiles to `insert_rows` + `write_matrix` | `crates/spreadsheet-kit/src/cli/commands/write.rs::append_region` | `crates/spreadsheet-kit/tests/cli_integration.rs` |
